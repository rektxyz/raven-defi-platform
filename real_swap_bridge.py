...
async function launchMeme() {
  ...
  try {
    ...
    const contract = await new ethers.ContractFactory(getERC20ABI(), getERC20Bytecode(), new ethers.providers.Web3Provider(window.ethereum).getSigner()).deploy(name, sym, ethers.BigNumber.from(supply));
